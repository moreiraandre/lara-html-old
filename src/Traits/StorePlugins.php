...
trait StorePlugins
{
     ...
     * Neste contexto $this é o plugin container.
     *
     * @param $name
     * @param $arguments
     * @return mixed
...
//                if ($obj->getPluginName() != "ButtonGroup")
//                    dd($this->config('plugins.'.$this->getPluginName().'.config-storeRows'), $obj);
            } catch (\TypeError $e) {
                throw new LaraHtmlParametersException($e->getMessage());
            }

            if ($this instanceof Plugin) {
//                dd('teste', $this, $this->config('plugins.' . $this->getPluginName() . '.config-storeRows'));
                if ($this->config('plugins.' . $this->getPluginName() . '.config-storeRows')) {
